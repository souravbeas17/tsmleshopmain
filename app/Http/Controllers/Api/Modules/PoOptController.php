...
public function getCamCustomer()
    ...
    public function getRejectedOrderAdmin()
    {
    	try
    	{
            ...
            ->leftjoin('quote_schedules','quote_schedules.quote_id','quotes.id')
            ->leftjoin('sub_categorys','quote_schedules.sub_cat_id','sub_categorys.id')
            ->select('quotes.rfq_no','quotes.quantity','quote_schedules.pro_size','quote_schedules.to_date','quote_schedules.from_date','sub_categorys.sub_cat_name as pro_name','quote_schedules.remarks','quote_schedules.kamsRemarks')
            ->whereNull('quotes.deleted_at')
            ->groupBy('schedule_no')
            ->orderBy('quotes.created_at','desc')
            ->get()->toArray();
